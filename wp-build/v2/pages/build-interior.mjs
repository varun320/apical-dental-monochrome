...
// V2 interior page builder. Reads v2 navbar + cta as shared chrome, builds
// hero + body sections per slug. White palette. Outputs slug.json files
// in this dir, and slug.escaped.txt via escape-pages.mjs.

const __here = dirname(fileURLToPath(import.meta.url));
const sectionsDir = resolve(__here, '..', 'sections');
...
const footer = JSON.parse(readFileSync(resolve(sectionsDir, '08-footer.json'), 'utf-8'));
  ...
  elements: [
    {
      ...
      elements: blocks.map((b, i) => ({
        ...
        settings: {
          editor: '<div style=\"font-family:Inter;max-width:720px;margin:0 auto;border-top:1px solid rgba(26,26,28,0.14);padding-top:32px\"><div style=\"font-size:11px;font-weight:500;letter-spacing:2px;text-transform:uppercase;color:rgba(26,26,28,0.5);margin-bottom:18px\">' + b.kicker + '</div><h3 style=\"font-family:Fraunces;font-size:1.85rem;font-weight:400;letter-spacing:-0.5px;color:#1A1A1C;margin:0 0 18px;line-height:1.15\">' + b.title + '</h3><p style=\"font-size:1rem;line-height:1.7;color:rgba(26,26,28,0.75);margin:0\">' + b.body + '</p></div>',
...
for (const [slug, page] of Object.entries(pages)) {
  // V2 prefix per page so all elementor ids are unique across pages and across v1
  const prefix = 'v2p' + slug.replace(/-/g, '') + '_';
  // Clone shared chrome and re-prefix their ids per-page
  const navClone = JSON.parse(JSON.stringify(nav));
  const ctaClone = JSON.parse(JSON.stringify(cta));
  const footClone = JSON.parse(JSON.stringify(footer));
  // Replace v2 prefixes inside cloned chrome with per-page prefix to avoid collisions
  const reprefix = (obj) => {
    if (!obj || typeof obj !== 'object') return;
    if (typeof obj.id === 'string' && obj.id.startsWith('v2')) {
      obj.id = prefix + obj.id.slice(2);
    }
    if (Array.isArray(obj.elements)) obj.elements.forEach(reprefix);
  };
  reprefix(navClone); reprefix(ctaClone); reprefix(footClone);
  ...
  writeFileSync(resolve(__here, slug + '.json'), JSON.stringify(data));
  console.log('v2 ' + slug, '→', JSON.stringify(data).length, 'bytes');
}
