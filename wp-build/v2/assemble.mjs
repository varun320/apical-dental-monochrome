...
// V2 assemble — reads wp-build/v2/sections/*.json (white-palette, v2-prefixed ids),
// emits wp-build/v2/home-elementor.json + compact text.
...
const collectIds = (node) => {
  ...
  if (typeof node.id === 'string') {
    if (ids.has(node.id)) throw new Error(`Duplicate id in v2: ${node.id}`);
    ids.add(node.id);
  }
...
console.log(`V2 assembled ${sections.length} sections, ${ids.size} unique ids.`);
